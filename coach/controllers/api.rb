...
Szcgs::Coach.controllers :v1 do  
  ...
  before :except => [:login, :unlogin, :logout, :about, :letter, :pay_record, :rank] do 
	...
	get :rank do 
		...
		@ranks = TeacherRank.all(:month => last_month.strftime('%Y%m'), :type => params[:type], :order => [:val.desc, :total_score.desc], :limit => 30)
		render 'rank'
	end

	get :withdraw_logs do
    if params[:status].present?
    	@logs = @teacher.withdraw_logs.all( :status => params[:status].to_i)
			@total = @logs.count
      @logs = @logs.paginate(:page => params[:page], :per_page => 20)
      render 'withdraw_logs'
		end
  end

	post :withdraw do
    if @teacher.withdraw_money>0 && params[:amounts].to_i>@teacher.withdraw_money
			{:status => :failure, :msg => '取现金额大于可取金额'}.to_json
    else
			new_log = WithdrawLog.new(:amounts => params[:amounts].to_i, :teacher_id =>@teacher.id)
      if new_log.save
				@teacher.withdraw_money -= params[:amounts].to_i
				if @teacher.save
					{:status => :success, :msg => '申请提现成功'}.to_json
        end
      end
    end
  end
  ...
  get :freeze do
    @orders = @teacher.orders.all( :status => 104, :type => 0)
    @total = @orders.count
